...
for i, item in enumerate(items):
    ...
    # case2) 멀티탭에 빈 구멍이 없고 현재 item이 멀티탭에 꽃혀있지 않은 경우 -> 스케줄링 필요
        ...
        for j in range(i+1, k):  # 아직 처리되지 않은 item들 순회
            if items[j] in tmp:  # 남아있는 item중에 지금 확인중인 item이 멀티탭에 꽂혀있고 그 item이 멀티탭에 꽂혀 있다는 것을 처음 알게된 경우
        ...
        if len(tmp) == 0:  # 현재 멀티탭에 꽃혀 있는 item들 전부 아직 처리되지 않은 item들에 포함되어 있다면(제거가능한 item이 없다면), 가장 나중에 등장하는 제품 먼저 제거함
            using.remove(last)
        # 현재 멀티탭에 꽂혀있는 item들 중에 이후 두번 다시 등장하지 않는 제품들이 있는 경우(tmp에 존재하는 item들), 해당 item 제거
        else:
            using.remove(tmp[0])
        using.append(item)
        change += 1
